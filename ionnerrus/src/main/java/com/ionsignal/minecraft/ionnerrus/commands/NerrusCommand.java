 ...
 * if (args.length == 0) {
 * sender.sendMessage(
 * Component.text("Usage: /nerrus <spawn|remove|stop|gather|give|do|ask|list|follow|craft> ...",
 * NamedTextColor.GOLD));
 * return true;
 * }
 ...
 * sender.sendMessage(
 * Component.
 * text("Unknown command. Usage: /nerrus <spawn|remove|stop|gather|give|do|ask|list|follow|craft> ..."
 * ,
 * NamedTextColor.RED));
 * return true;
 * }
 ...
 * if (groundBlock.getType().isSolid() && !feetBlock.getType().isSolid() &&
 ...
 * player.sendMessage(Component.text("An agent with that name already exists.",
 * NamedTextColor.RED));
 * return true;
 * }
 * Location safeLoc = findSafeSpawningLocation(player.getLocation());
 * UUID fakeDefinitionId = UUID.randomUUID();
 * UUID fakeSessionId = UUID.randomUUID();
 * UUID ownerId = player.getUniqueId();
 * SpawnAgentCommand command = new SpawnAgentCommand(
 * name,
 * safeLoc,
 * null,
 * fakeDefinitionId,
 * fakeSessionId,
 * ownerId);
 * agentService.spawnAgent(command);
 ...
 * } else {
 * sender.sendMessage(Component.text("Failed to remove agent: " + name, NamedTextColor.RED));
 * }
 * } else {
 * sender.sendMessage(Component.text("Agent has no session ID (cannot despawn via new API): " +
 ...
 * sender.sendMessage(Component.text("Stopped current goal for agent " + name,
 ...
 * if (args.length < 4) {
 * sender.sendMessage(Component.text("Usage: /nerrus gather <name> <type> <amount>",
 * NamedTextColor.RED));
 * return true;
 * }
 * String name = args[1];
 ...
 * text("Usage: /nerrus give <agentName> <targetName> <material> <quantity>", NamedTextColor.RED));
 * return true;
 * }
 ...
 * sender.sendMessage(Component.text("Instructing " + agentName + " to give items.",
 ...
 * if (args.length < 4) {
 * sender.sendMessage(Component.text("Usage: /nerrus craft <agent_name> <item_name> <quantity>",
 * NamedTextColor.RED));
 * return true;
 * }
 * String agentName = args[1];
 ...
 * sender.sendMessage(Component.text("Instructing " + agent.getName() + " to craft " + quantity +
 * " " + itemName + ".",
 * NamedTextColor.GREEN));
 * } catch (IllegalArgumentException e) {
 * sender.sendMessage(Component.text("Error creating goal: " + e.getMessage(), NamedTextColor.RED));
 ...
 * if (args.length < 3) {
 * player.sendMessage(Component.text("Usage: /nerrus do <name> <directive...>",
 * NamedTextColor.RED));
 * return true;
 * }
 * String name = args[1];
 ...
 * player.sendMessage(Component.text("Directive issued to " + name + ": '" + directive + "'",
 * NamedTextColor.GREEN));
 * return true;
 * }
 * 
 ...
 * if (args.length < 3) {
 * player.sendMessage(Component.text("Usage: /nerrus ask <name> <question...>",
 * NamedTextColor.RED));
 * return true;
 * }
 * String name = args[1];
 ...
 * player.sendMessage(Component.text("Asking " + name + ": '" + question + "'",
 * NamedTextColor.GRAY));
 ...
 * sender.sendMessage(Component.text("Instructing " + agent.getName() + " to follow " + targetName,
 * NamedTextColor.GREEN));
 * } catch (IllegalArgumentException e) {
 ...
 * command, @NotNull String alias,
 * 
 * @NotNull String[] args) {
 * if (args.length == 1) {
 * return List.of("spawn", "remove", "stop", "gather", "give", "do", "ask", "list", "follow",
 * "craft");
 * }
 * if (args.length == 2) {
