package com.ionsignal.minecraft.ionnerrus.terra.core;
...
public final class JigsawConnection {
	...
	/**
	 * Checks if two jigsaw blocks can connect to each other.
	 * 
	 * @param source
	...
	public static boolean canConnect(JigsawData.JigsawBlock source, JigsawData.JigsawBlock target) {
		...
		if (!areOrientationsCompatible(source.orientation(), target.orientation())) {
			return false;
		}
	...
	private static String getOppositeOrientation(String orientation) {
		return switch (orientation.toLowerCase()) {
			case "north", "north_up" -> "south";
			case "south", "south_up" -> "north";
			case "east", "east_up" -> "west";
			case "west", "west_up" -> "east";
			case "up" -> "down";
			case "down" -> "up";
			default -> orientation; // Unknown orientation
		};
	}
	 ...
	 */
	public static boolean areJointTypesCompatible(JigsawData.JointType sourceJoint,
			JigsawData.JointType targetJoint) {
		// Both must be the same type, or one must be rollable
		// Rollable joints are more flexible
