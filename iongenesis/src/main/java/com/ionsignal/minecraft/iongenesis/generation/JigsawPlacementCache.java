...
public class JigsawPlacementCache {
    ...
    private final ConcurrentHashMap<PlacementCacheKey, StructureBlueprint> cache = new ConcurrentHashMap<>();
     ...
     * @param generator
     *            Supplier that generates the blueprint if not cached
     * @return The cached or newly generated blueprint
     */
    public StructureBlueprint getOrGenerate(PlacementCacheKey key, Supplier<StructureBlueprint> generator) {
        // Fast path: check if already cached
        StructureBlueprint cached = cache.get(key);
        if (cached != null) {
            return cached;
        }
        ...
        try {
            ...
            StructureBlueprint generated = generator.get();
            // Store in cache
